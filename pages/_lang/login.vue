  ...
  <div class="content">
    <div class="pane-form">
      ...
      <div class="button-wrapper">
        <button @click="onLogin" class="button">{{$t('pages.sign.login')}}</button>
      </div>
      <div class="hint">
        <nuxt-link :to="'/' + $i18n.locale + '/signup'" class="link" title="Create free account">{{$t('pages.sign.create_free_account')}}</nuxt-link>
      </div>
    </div>
  </div>
</template>
